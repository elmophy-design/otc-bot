...
class SharedMarketScanner:
    """Read-only market scanner using the bot's existing MarketDataFetcher.

    This class deliberately does NOT create a broker connection.  The caller
    injects the already-connected fetcher owned by the Telegram bot process.
    """
    ...
    async def _scan_asset(self, asset: str, timeframe: str) -> dict[str, Any]:
        ...
        df = await self.fetcher.get_historical_data(
            asset, timeframe=timeframe, count=_DEFAULT_COUNT
        )
        ...
        if not required.issubset(df.columns):
            raise RuntimeError(
                f"missing OHLC columns: {sorted(required - set(df.columns))}"
            )
        ...
        rsi_score = 15 if (
            (trend == "BULLISH" and 50 <= rsi <= 70)
            or (trend == "BEARISH" and 30 <= rsi <= 50)
        ) else 8
        macd_score = 15 if (
            (trend == "BULLISH" and m > ms)
            or (trend == "BEARISH" and m < ms)
        ) else 6
        ...
        price_score = 10 if (
            (move > 0 and trend == "BULLISH")
            or (move < 0 and trend == "BEARISH")
        ) else 5
    ...
    def _validate_timeframe(timeframe: str) -> str:
        value = str(timeframe or "1m").lower().strip()
