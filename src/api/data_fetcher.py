...
class MarketDataFetcher:
    ...
    async def get_historical_data(
        ...
        # Freshness check for live data (reject candles older than ~3 minutes)
        if df is not None and not df.empty and "timestamp" in df.columns:
            try:
                ...
                if age_sec > 180:
                    logger.warning(
                        "Live data for %s is stale (%.0f s old) – discarding",
                        asset,
                        age_sec,
                    )
                    if not self.demo_mode:
                        raise RuntimeError(
                            f"Stale market data for {asset} ({age_sec:.0f}s old)"
                        )
                    df = None
            except RuntimeError:
                raise
        ...
        try:
            if hasattr(self.client, "get_candles"):
                df = await asyncio.wait_for(
                    self.client.get_candles(asset, timeframe=timeframe, count=count),
                    timeout=18.0,
                )
            ...
            logger.warning(
                "Live candle fetch for %s exceeded 18s overall timeout; using fallback",
                asset,
            )
            return None
