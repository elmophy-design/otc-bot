...
class PocketOptionClient:
    ...
    # ------------------------------------------------------------------
    # Trade execution is NOT implemented by this reverse-engineered
    # Socket.IO client — placing real orders needs the authenticated
    # order-placement frames, which aren't reproduced here. Fail
    # loudly (but without crashing) instead of pretending to trade.
    # Use LIVE_BACKEND=botv2 (BinaryOptionsToolsV2) for real execution.
    # ------------------------------------------------------------------
    async def place_trade(
        self, asset: str, direction: str, amount: float, duration: int = 60
    ) -> Dict[str, Any]:
        return {
            "success": False,
            "order_id": None,
            "error": (
                "Trade execution is not supported by the native client. "
                "Set LIVE_BACKEND=botv2 and install BinaryOptionsToolsV2."
            ),
        }

    async def check_win(self, order_id: str, timeout_seconds: int = 90) -> Dict[str, Any]:
        return {
            "success": False,
            "result": None,
            "error": "check_win not supported by the native client.",
        }
