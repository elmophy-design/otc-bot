...
class SignalEngine:
    ...
    def _analyze(
        ...
        # 5b. Extreme RSI + Stochastic cluster (strongest OTC mean-reversion edge)
        if rsi >= 80 and stoch_k >= 85:
            votes["rsi"] = -1
            votes["stochastic"] = -1
            reasons.append(f"Extreme overbought cluster (RSI {rsi:.0f} + Stoch {stoch_k:.0f})")
        elif rsi <= 20 and stoch_k <= 15:
            votes["rsi"] = 1
            votes["stochastic"] = 1
            reasons.append(f"Extreme oversold cluster (RSI {rsi:.0f} + Stoch {stoch_k:.0f})")
        ...
        snapshot = {
            ...
            "adx": round(adx, 2),
            "atr": round(last(ind.get("atr"), 0.0), 6),
        }

        # --- Volatility filter (skip dead / flat markets) ---
        atr_val = last(ind.get("atr"), 0.0)
        bb_width = (bb_upper - bb_lower) / bb_mid if bb_mid > 0 else 0.0
        rel_atr = atr_val / last_close if last_close > 0 else 0.0

        if bb_width < 0.0008 or rel_atr < 0.00015:
            return {
                "action": "NO_SIGNAL",
                "confidence": 0,
                "reason": (
                    f"Low volatility filter (BB width={bb_width:.5f}, "
                    f"rel ATR={rel_atr:.5f}). Market too quiet."
                ),
                "votes": votes,
        ...
        # Decision thresholds – majority + minimum agreement
        # Requires at least `min_confluence` votes on one side AND a clear majority.
        # Allows clean 3-2 and strong 2-0 / 2-1 setups while still filtering pure 50/50.
        min_conf = self.min_confluence
        if bullish >= min_conf and bullish > bearish:
            action = "CALL"
            reason = " | ".join(reasons) if reasons else "Bullish confluence"
        elif bearish >= min_conf and bearish > bullish:
            action = "PUT"
